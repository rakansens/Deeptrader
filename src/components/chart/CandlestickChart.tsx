...
export default function CandlestickChart({
  ...
  useEffect(() => {
    ...
    const toRgb = (name: string, fallback: string) => {
      const v = styles.getPropertyValue(name).trim();
      return v ? `rgb(${v})` : fallback;
    };
    const bg = toRgb("--tv-bg", theme === "dark" ? "#131722" : "#ffffff");
    const grid = toRgb("--tv-grid", theme === "dark" ? "#2a2e39" : "#e8e8e8");
    const cross = toRgb(
      "--tv-crosshair",
      theme === "dark" ? "#5c5c5c" : "#989898",
    );
  ...
  useEffect(() => {
    ...
    const ensureSeries = (
      cond: boolean,
      ref: React.MutableRefObject<ReturnType<
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        raw.forEach((d: any) => {
          ...
          if (macd) {
            macdLineDataRef.current.push({
              time: candle.time,
              value: macd.macd,
            });
            signalLineDataRef.current.push({
              time: candle.time,
              value: macd.signal,
            });
          }
          ...
          if (boll) {
            bollUpperDataRef.current.push({
              time: candle.time,
              value: boll.upper,
            });
            bollLowerDataRef.current.push({
              time: candle.time,
              value: boll.lower,
            });
          }
        ...
        if (bollUpperSeriesRef.current)
          bollUpperSeriesRef.current.setData(bollUpperDataRef.current);
        if (bollLowerSeriesRef.current)
          bollLowerSeriesRef.current.setData(bollLowerDataRef.current);
      } catch (e) {
        console.error(e);
      }
    }
    ...
    ws.onmessage = (ev) => {
      ...
      if (macd && macdSeriesRef.current && signalSeriesRef.current) {
        ...
        signalSeriesRef.current.update({
          time: candle.time,
          value: macd.signal,
        });
      }
      ...
      if (boll && bollUpperSeriesRef.current && bollLowerSeriesRef.current) {
        bollUpperSeriesRef.current.update({
          time: candle.time,
          value: boll.upper,
        });
        bollLowerSeriesRef.current.update({
          time: candle.time,
          value: boll.lower,
        });
      }
  ...
  const getChartHeights = () => {
    const mainChartHeight =
      indicators.rsi || indicators.macd ? height * 0.6 : height;
    const subPanels = [];
  ...
  return (
    <div className={cn("bg-background border border-border p-2", className)}>
