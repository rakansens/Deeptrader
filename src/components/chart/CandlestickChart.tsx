...
export default function CandlestickChart({
  ...
  useEffect(() => {
    ...
    const storedCandles = localStorage.getItem(
      `candles_${initialSymbol}_${initialInterval}`,
    );
    const storedVolumes = localStorage.getItem(
      `volumes_${initialSymbol}_${initialInterval}`,
    );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        ...
        const processedCandles = processTimeSeriesData<CandlestickData>(
          candles,
          timeToNumber,
        );
      ...
      if (
        ...
        const processedVolumes = processTimeSeriesData<HistogramData>(
          volumes,
          timeToNumber,
        );
    ...
    (el: HTMLDivElement) => {
      ...
      if (chartRef.current) {
        ...
        return () => {
          chartRef.current
            ?.timeScale()
            .unsubscribeVisibleLogicalRangeChange(sub);
          rsiChart.remove();
  ...
  const mainChartHeight =
    height -
    (indicators.rsi ? subPanelHeight : 0) -
    (indicators.macd ? subPanelHeight : 0);
