...
export const chartAnalysisTool = createTool({
  ...
  inputSchema: z.object({
    ...
    indicators: z.array(z.string()).optional().describe("計算するテクニカル指標のリスト (例: ['RSI', 'MACD', 'Bollinger'])"),
  ...
  execute: async ({ context }) => {
    ...
    return {
      ...
      indicators: indicators.map(indicator => {
        if (indicator === "RSI") {
          ...
          return {
            name: "RSI",
            value,
            interpretation: value < 30 ? "過売り" : value > 70 ? "過買い" : "中立",
          ...
          return {
            ...
            interpretation: histogram > 0 
              ? "上昇トレンド形成の可能性" 
              : "下降トレンド形成の可能性",
            signal: histogram > 0 && histogram > Math.abs(histogram * 0.1) 
              ? "買い" 
              : histogram < 0 && Math.abs(histogram) > Math.abs(histogram * 0.1) 
                ? "売り" 
                : "中立"
          };
          ...
          return {
            ...
            interpretation: ma50 > ma200 
              ? "ゴールデンクロス形成" 
              : ma50 < ma200 
                ? "デッドクロス形成" 
                : "クロスなし",
            signal: ma50 > ma200 && currentPrice > ma50 
              ? "強気トレンド" 
      ...
      patterns: patternDetection ? [
        {
          name: ["ダブルトップ", "ダブルボトム", "頭と肩", "逆頭と肩", "三角形", "旗", "ペナント"][Math.floor(Math.random() * 7)],
