...
const chartAnalysisServerTool = createTool({
  ...
  inputSchema: z.object({
    ...
    indicators: z.array(z.string()).optional().describe("計算するテクニカル指標のリスト"),
    patternDetection: z.boolean().optional().describe("チャートパターン検出を実行するかどうか")
  }),
  execute: async ({ context }) => {
    const { symbol, timeframe, indicators = [], patternDetection = false } = context;
    ...
    return {
      ...
      patterns: patternDetection ? [
        {
          name: ["ダブルトップ", "ダブルボトム", "三角形", "ヘッドアンドショルダー"][Math.floor(Math.random() * 4)],
...
const marketDataServerTool = createTool({
  ...
  inputSchema: z.object({
    ...
    dataType: z.enum(["price", "volume", "orderbook", "funding", "all"]).describe("取得するデータタイプ")
  }),
  execute: async ({ context }) => {
    ...
    return {
      ...
      timestamp: new Date().toISOString(),
      data: {
        price: dataType === "price" || dataType === "all" ? {
        ...
        volume: dataType === "volume" || dataType === "all" ? {
